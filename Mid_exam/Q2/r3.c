...
char *path1 = "./fsock3"; // for at

char* path2 = "./fsock6"; // for R2
char* path3 = "./fscok5"; // for R1
...
int main() 
{
    ...
    sleep(1);
    ufdr2 = cli_conn(name3,path3);
    sleep(1);
    char conf[] = "Linked with R3";
    fd_set rfd;
    fd_set wfd;
    fd_set erfd;
    FD_ZERO(&rfd);
    ...
    while(1)
    {
        ...
        maxfd=ufd;
        maxfd=max(ufdr1,maxfd);
        maxfd=max(ufdr2,maxfd);
        for(int i=0;i<used;i++)
        {
            if(nsfd[i]>maxfd)
                maxfd=nsfd[i];
        ...
        if(FD_ISSET(ufdr2,&rfd))
        {
            if((nsfd[used] = recv_fd(ufdr2)) < 0) 
            {
                perror("recv_fd");
                exit(1);
            }
            else
                printf("Received [fd == %d] from R2\n",nsfd[used]);
            used++;
            continue;
        }
        for (int i = 0; i < used; i++)
        {
            if (FD_ISSET(nsfd[i],&rfd))
            {
                ...
                if(st.rounds<=2) // Not the last interview round
                {
                    if(st.sequence[st.rounds]==2)
                    {
                    ...
                    else if(st.sequence[st.rounds]==1)
                    {
                ...
                else // This is the last interview round
                {
                    //sending to J
                    char* name="r3toj";
